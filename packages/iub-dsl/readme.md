...
| 相杰 | 2020-06-22 | v0.0.2 | 1. 修改页面间交互时序图 <br> 2. 添加更详细的 IUB-DSL 业务领域模型图 <br> 3. 添加更详细的名词解释 <br> 4. 工作原理图添加数据变更时通知接口的链路 |
...
1. 将「业务」按照一定的数据结构拆分成不同的 IUB-DSL 节点进行描述
2. 通过规定节点之间的关联关系，保证 IUB-DSL 对业务的描述的完整性
3. 每个节点都是单一职能，可以通过其中的节点关系满足业务扩展的需求
...
![图片描述](/tfl/pictures/202006/tapd_41909965_1592788578_42.png)
...
![图片描述](/tfl/pictures/202006/tapd_41909965_1592619040_92.png)

### IUB-DSL 运行模型

IUB-DSL Entity（实例）本质上是一种结构化数据，每个节点遵循单一职责原则（SRP），只对一种类型的任务或实体负责：

![图片描述](/tfl/pictures/202006/tapd_41909965_1592791021_90.png)
...
- 运行容器：目前主要有三种运行容器：
  1. 应用运行容器：整个应用平台前端运行容器，用于支撑系统运行
  2. 页面运行容器：用于根据页面类型（配置、定制）的不同而选择不同页面加载方式的容器
  3. 流程运行容器：用于页面内，当数据发生更改时，用于运算数据更改事务的容器

-----

IUB-DSL 在运行时一共会产生三种「运行时上下文」：

1. 系统运行时上下文（system runtime context简称 SysRtCtx），由应用运行容器产生。
2. 页面运行时上下文（page runtime context 简称 PgRtCtx），由页面运行容器产生。
3. 流程运行时上下文（flow runtime context 简称 FlRtCtx），由流程运行容器产生。

上下文的交互关系和规则：

- 运行时数据传递由上到下，不允许跨级通讯。
- 「页面运行时上下文 PgRtCtx」是平行关系，统一由 SysRtCtx 调度数据交互。
- 「流程运行时上下文 FlRtCtx」在 PgRtCtx 之中，是业务运行时的数据流转载体，具备事务性质。
...
在 IUB-DSL 中，「页面」之间的关系是平行的，无论是内嵌页面还是弹窗页面，在 IUB-DSL 运行时都在「系统运行时上下文之下」。

![图片描述](/tfl/pictures/202006/tapd_41909965_1592791038_19.png)

为了实现页面之间的解藕以及通讯，这里采用订阅发布模式，统一由「数据调度器」提供订阅服务，页面之间通讯依赖「数据调度器」。
...
![图片描述](/tfl/pictures/202006/tapd_41909965_1592805786_62.png)

-----

IUB-DSL 模型完整版

![图片描述](/tfl/pictures/202006/tapd_41909965_1592805803_82.png)